...
import { SystemType, useToggleSystemCheckMutation } from '@/entities/work'
...
interface SystemCheckContentProps {
  systems: SystemItem[]
  onToggle: () => void
}

export const SystemCheckContent = ({ systems, onToggle }: SystemCheckContentProps) => {
  return (
      ...
      <CardContent className="flex-1 p-0 min-h-0">
        <div className="h-full overflow-y-auto">
          <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-2 p-2">
            {systems.map((system) => (
              <SystemRow key={system.id} system={system} onToggle={onToggle} />
            ))}
          </div>
        </div>
      </CardContent>
