...
import EmptyWorkStatus from '@/features/home/dashboard/components/widgets/EmptyWorkStatus';
import TodayWorkStatusWidgetSkeleton from '@/features/home/dashboard/components/widgets/TodayWorkStatusWidgetSkeleton';
...
const TodayWorkStatusWidget = () => {
  ...
  const { total_hours = 0, required_hours = 1, completed = false } = status || {};
  const percentage = Math.min((total_hours / required_hours) * 100, 100);

  return (
    <QueryAsyncBoundary
      queryState={{ isLoading, error, data: status }}
      loadingComponent={<TodayWorkStatusWidgetSkeleton />}
      emptyComponent={
